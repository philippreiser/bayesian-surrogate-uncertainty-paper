...
data {
  ...
  vector[N_clusters] sigma_sim; // sigma from surrogate training step
...
model {
  ...
  if (!prior_only) {
    ...
    for (k in 1:N_clusters) {
      vector[N_exp] y_exp_pce =  c_0[k] + w_exp_pce*to_vector(c[k, :]);
      for (n in 1:N_exp) {
        target += cluster_weights[k] * normal_lpdf(to_vector(y_exp[n, :]) | y_exp_pce[n], sqrt(square(sigma_exp)+square(sigma_sim[k])));
      }
    }
  }
  // Prior model
...
generated quantities {
  matrix<lower=-1, upper=1>[N_exp, M] w_prior;
  w_prior = w_exp;
}
